  ...
  static plugin(plugin) {
    ...
    if (typeof plugin === 'object' && !!plugin.name) {
      ...
      if (({}).hasOwnProperty.call(this, name))
        throw new WryError(`Plugin name \`${name}\` is already registered.`);

      this.Continue.plugin(plugin);
      this.prototype[name] = function (...args) {
        // Create an unresolved Continue
        const cont = new Wry.Continue();
        // Call the specific plugin method
    ...
    throw new WryError(`Plugin is of an invalid type`);
  }
}
